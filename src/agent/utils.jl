...
#function find_μ(Σ, y) # W is of the proposed mean. Because we can get the precision as the Hessian of the log likelihood
#    iter = 10
#    # Make outputs -1/1 instead of 0/1
#    y_scaled = (y .- 0.5) * 2.0
#    μ_test = zeros(size(y, 1))
#
#    for i = 1:iter # Need to make a check for convergence here
#	logp,∇_logp,∇∇_logp = ∇_log_prob(y_scaled,μ_test)
#	W_ = Diagonal( -∇∇_logp)
#	#μ_test = Σ * a
#	μ_test = inv(Σ + inv(W_)) * (W_*μ_test + ∇_logp)
#	# Objective for convergence
##	obj_new = -0.5*a'*(μ_test) + sum(logp)
#			       #.- log.(diag(L.L)))
